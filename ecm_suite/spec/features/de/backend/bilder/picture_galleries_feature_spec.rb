...
RSpec.describe '/de/backend/bilder/picture_galleries', type: :feature, locale: :de do
  ...
  describe 'appending picture details' do
    let(:original_assets) {[
      { io: File.open(Ecm::Galleries::Engine.root.join(*%w(spec files ecm galleries picture_details example.png))), filename: 'example.png'},
      { io: File.open(Ecm::Galleries::Engine.root.join(*%w(spec files ecm galleries picture_details example.png))), filename: 'example.png'}
    ]}
    let(:resource) { create(:ecm_galleries_picture_gallery, assets: original_assets) }
  ...
  describe 'replacing picture details' do
    let(:original_assets) {[
      { io: File.open(Ecm::Galleries::Engine.root.join(*%w(spec files ecm galleries picture_details example.png))), filename: 'example.jpg'},
      { io: File.open(Ecm::Galleries::Engine.root.join(*%w(spec files ecm galleries picture_details example.png))), filename: 'example.jpg'}
    ]}
    let(:resource) { create(:ecm_galleries_picture_gallery, assets: original_assets) }
    let(:base_path) { '/de/backend/bilder/picture_galleries' }
