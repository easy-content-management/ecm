...
RSpec.describe '/de/backend/dateien/folders', type: :feature do
  ...
  describe 'appending item details' do
    let(:original_assets) {[
      { io: File.open(Ecm::Files::Engine.root.join(*%w(spec files ecm files file_details example.png))), filename: 'example.png'},
      { io: File.open(Ecm::Files::Engine.root.join(*%w(spec files ecm files file_details example.png))), filename: 'example.png'}
    ]}
    let(:resource) { create(:ecm_files_folder, assets: original_assets) }
