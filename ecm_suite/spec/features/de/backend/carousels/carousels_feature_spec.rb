...
RSpec.describe '/de/backend/carousels/carousels', type: :feature do
  ...
  describe 'appending item details' do
    let(:original_assets) {[
      { io: File.open(Ecm::Carousels::Engine.root.join(*%w(spec files ecm carousels item_details example.png))), filename: 'example.png'},
      { io: File.open(Ecm::Carousels::Engine.root.join(*%w(spec files ecm carousels item_details example.png))), filename: 'example.png'}
    ]}
    let(:resource) { create(:ecm_carousels_carousel, assets: original_assets) }
