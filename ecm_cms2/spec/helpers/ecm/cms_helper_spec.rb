  ...
  describe CmsHelper do
    describe '#cms_render_navigation' do
      ...
      it 'shows an warning on empty navigation' do
        name = 'main'
        navigation = FactoryBot.create :ecm_cms_navigation, name: name, locale: I18n.locale.to_s
      ...
      describe 'options' do
        before(:each) do
          @navigation = FactoryBot.create(:ecm_cms_navigation, locale: I18n.locale)
          @items = [FactoryBot.create(:ecm_cms_navigation_item, ecm_cms_navigation: @navigation, data_add_icon: 'foo')]
        end
