...
feature 'Ecm::Cms::Template admin' do
  ...
  def set_admin_area_path
    @admin_area_path = "/#{I18n.locale}/admin"
  end

  def set_resource_path
    @resource_path = 'ecm_cms_templates'
  end

  def set_resource_class
    @resource_class = Ecm::Cms::Template
  end
  ...
  def fill_new_form
    fill_in 'ecm_cms_template[pathname]', with: '/'
    fill_in 'ecm_cms_template[basename]', with: '_example_template'
    select 'erb', from: 'ecm_cms_template[handler]'
  end

  def fill_edit_form
    fill_in 'ecm_cms_template[basename]', with: '_changed_template_partial'
  end
