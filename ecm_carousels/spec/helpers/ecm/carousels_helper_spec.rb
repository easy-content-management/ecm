  ...
  describe CarouselsHelper do
    describe "#render_carousel" do
      context "basic behaviour" do
        before(:each) do
          @carousel = FactoryBot.create(:ecm_carousels_carousel_with_items)
        end

        subject { helper.render_carousel(@carousel.name) }

        it "should include the body of all items" do
          @carousel.ecm_carousels_items.each do |item|
      ...
      context "uses carousel without locale if there is no better" do
        before(:each) do
          @carousel = FactoryBot.create(:ecm_carousels_carousel_with_items, :locale => nil)
        end
